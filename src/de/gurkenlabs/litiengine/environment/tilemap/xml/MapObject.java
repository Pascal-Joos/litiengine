...
public class MapObject extends CustomPropertyProvider implements IMapObject {
  ...
  public float getX() {
    if (this.isInfiniteMap()) {
      TmxMap map = (TmxMap) this.getLayer().getMap();
      float localX = this.x == null ? 0 : this.x;
      return localX - map.getChunkOffsetX() * map.getTileWidth();
    }
  ...
  public float getY() {
    if (this.isInfiniteMap()) {
      TmxMap map = (TmxMap) this.getLayer().getMap();
      float localY = this.y == null ? 0 : this.y;
      return localY - map.getChunkOffsetY() * map.getTileHeight();
    }
