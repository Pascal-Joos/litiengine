...
public class MapObject extends CustomPropertyProvider implements IMapObject {
  ...
  public float getX() {
    if (this.isInfiniteMap()) {
      TmxMap map = (TmxMap) this.getLayer().getMap();
      float baseX = this.x == null ? 0f : this.x;
      return baseX - map.getChunkOffsetX() * map.getTileWidth();
    }
  ...
  public float getY() {
    if (this.isInfiniteMap()) {
      TmxMap map = (TmxMap) this.getLayer().getMap();
      float baseY = this.y == null ? 0f : this.y;
      return baseY - map.getChunkOffsetY() * map.getTileHeight();
    }
  ...
  public IMapObjectLayer getLayer() {
    if (this.layer == null) {
      throw new IllegalStateException("MapObject layer is not set");
    }
    return this.layer;
  }
